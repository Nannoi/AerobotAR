...
namespace YourNamespace
{
    public class LineFormation : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            //robotIndex = Mathf.Abs(robotId.GetHashCode()) % Mathf.RoundToInt(totalRobots);
            robotIndex = int.Parse(robotId);
            FrobotIndex = FrobotId;
            positionRatio = (endPosition.x - startPosition.x) / (numIterations - 1f);

            // Initialize position
            currentPosition = new Vector3(startPosition.x + positionRatio * (robotIndex-(FrobotId-1)-1f), 0, centerLocation.z);

            StartCoroutine(Execute());
            StartCoroutine(RunLineFormation());
        }
        ...
        public IEnumerator Execute()
        {
            ...
            Debug.Log($"Line Formation executing for robot {robotId}");
            ...
            Debug.Log("LINE_FORMATION_COMPLETED");
        }

        private IEnumerator RunLineFormation()
        {
            ...
            for (int i = 1; i <= numIterations; i++)
            {
                ...
                foreach (GameObject neighbour in neighbours)
                {
                    PositionData positionData = neighbour.GetComponent<LineFormation>().ReceivePositionFromNeighbour(neighbour);
